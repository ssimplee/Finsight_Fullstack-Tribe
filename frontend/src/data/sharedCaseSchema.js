...
function buildVisualObservations(observation, caseForm) {
  ...
  if (observation?.findings?.length) {
    observation.findings.forEach((finding, index) => {
      observationEntries.push({
        id: `obs-visual-${index + 1}`,
        label: "OBSERVED",
        text: finding,
      });
    });
  ...
  caseForm.visibleSymptoms.forEach((symptom, index) => {
    observationEntries.push({
      id: `reported-visual-${index + 1}`,
      label: "USER-REPORTED",
      text: symptom,
    });
  });
...
function buildBehavioralObservations(caseForm) {
  return caseForm.behavioralSigns.map((sign, index) => ({
    id: `behavior-${index + 1}`,
    label: "USER-REPORTED",
    text: sign,
  }));
...
function buildAgentQuestions(followUpAnswers) {
  return Object.entries(followUpAnswers).map(([questionId, answer]) => ({
    ...
    numeric_answer:
      answer.value === "" || answer.value === undefined ? null : Number(answer.value),
    selected_answer: answer.choice || null,
